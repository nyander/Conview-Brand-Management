...
@section('content')
  ...
  <div class="flex justify-center">
    <div class="w-11/12 rounded-md mx-8 pb-4  ">
        <h2 class="text-3xl antialiased font-bold">Add Fabric Type</h2>
    </div>
  </div>
  <form method="POST" action="/fabrictypes"  enctype="multipart/form-data" onsubmit="myButton.disabled = true; return true;">      
    @csrf
    <div class="justify-center mx-auto	 w-11/12 mb-12"> 
      <div class="field-row">
        <input class="input mt-4 bg-gray-100 border-b-2 border-black" type="text" name="fabricName" id="fabricName" placeholder="Fabric Name" >
      </div>

      <div class="mt-8  description_details">
        <h1 class="mb-4 font-semibold text-lg">Description</h1>
        <textarea id="description" name="description" rows="5" cols="75" class="bg-gray-100 border-2 border-gray-500 p-4 quickDescription" placeholder="Enter text here"></textarea>
      </div>
